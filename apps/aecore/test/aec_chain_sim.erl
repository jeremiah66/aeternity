...
%%%===================================================================
%%%  Chain simulator parametrization
%%%===================================================================
...
%% Genesis block with state tree. Ability to instantiate parent chain by predefined genesis;
%%
%%
-spec genesis_state_param(map()) -> {aec_blocks:key_block(), aec_trees:trees()}.
genesis_state_param(Opts) ->
    maps:get(genesis_state_param, Opts, aec_block_genesis:genesis_block_with_state()).

-spec genesis_state_param(map(), {aec_blocks:key_block(), aec_trees:trees()}) -> map().
genesis_state_param(Opts, GenesisState = {Block, Trees}) ->
    Opts#{ genesis_state_param => GenesisState }.
